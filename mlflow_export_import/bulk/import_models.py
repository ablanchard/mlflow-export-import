...
def _import_models(client, input_dir, run_info_map, delete_model, import_source_tags, verbose, use_threads, filter_user, model_names, ignore_models):
    ...
    if ignore_models != "":
        models = list(filter(lambda m: m not in ignore_models.split(","), models))

    if model_names != "":
        models = list(filter(lambda m: m in model_names.split(","), models))
...
def import_all(client, input_dir, delete_model, use_src_user_id=False, import_source_tags=False, verbose=False, use_threads=False, filter_user="", model_names="", ignore_models=""):
    start_time = time.time()
    ...
    model_res = _import_models(client, input_dir, run_info_map, delete_model, import_source_tags, verbose, use_threads, filter_user, model_names, ignore_models)
    duration = round(time.time()-start_time, 1)
...
@opt_model_names
@opt_ignore_models
def main(input_dir, delete_model, use_src_user_id, import_source_tags, verbose, use_threads, filter_user, model_names, ignore_models):
    print("Options:")
