...
def _export_models(client, model_names, output_dir, notebook_formats, stages, export_run=True, use_threads=False, ignore_models=""):
    max_workers = os.cpu_count() or 4 if use_threads else 1
    start_time = time.time()
    model_names = bulk_utils.get_model_names(client, model_names)

    if ignore_models != "":
        model_names = list(filter(lambda m: m not in ignore_models.split(","), model_names))
...
def export_models(client, model_names, output_dir, notebook_formats=None, stages="", export_all_runs=False, use_threads=False, ignore_models=""):
    ...
    _export_models(client, model_names, os.path.join(output_dir,"models"), notebook_formats, stages, export_run=False, use_threads=use_threads, ignore_models=ignore_models)
    duration = round(time.time()-start_time, 1)
...
def main(models, output_dir, stages, notebook_formats, export_all_runs, use_threads, ignore_models):
    print("Options:")
